...
const TipsList = [
  ...
  },
  {
    ...
    desc: [
      {
        desc: '如四大、国有银行等在招聘信息中不分具体职能部门的话，可以不写求职意向。如需要填写求职意向，则建议一份简历只针对一个意向岗位。',
        title: '',
      },
      {
        title: '1.错误示范：过于宽泛',
      ...
      {
        title: '2.注意：',
        desc: '多个意向岗位出现在一份简历中，没有针对性，会给HR一个信息：这个求职者的职业规划十分混乱',
      },
      {
        title: '3.推荐写法：精准的职位名称/行业+职位名称',
  ...
  },
  {
    ...
    desc: [
      ...
      {
        title: ' 1.根据开发进度和任务分配，完成相应模块软件的设计、开发、编程任务；',

        desc: ' 进行程序单元、功能的测试，查出软件存在的缺陷并保证其质量； 按计划完成量化的工作要求，并能独立处理和解决所负责的任务。',
      },
      {
        title: '3.项目：xxxxxx servic',
          ...
          return (
            <Panel header={item.title} key={item.title}>
              <div className="Panel-text">
                {item.desc instanceof Array ? <DescItem data={item.desc} /> : <p>{item.desc}</p>}
              </div>
            </Panel>
          );
        })}
